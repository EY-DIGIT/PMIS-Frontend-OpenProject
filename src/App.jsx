...
    const canReadPayment = useCan("payment:read");
    const allowed = canViewProjects && canReadPayment;
    if (allowed) return children;
    if (projectId) {
...
    Approval Inbox). Some elevated app roles also need oversight access
    from Dashboard drill-downs, so allowRoles can include configured
    roles.json identities such as super_admin/admin. */
function RequireWorkflowRole({ role, allowRoles = [], children }) {
     const currentRole = useCurrentRole();
     if (userHasRole(tokenStore.getUser(), role) || allowRoles.includes(currentRole)) return children;
    return (
        <div style={{ padding: 32, textAlign: 'center', color: '#5a6680' }}>
            <h2 style={{ marginBottom: 8, color: '#173e77' }}>Access denied</h2>
            <p>Your role does not have permission to view this page.</p>
...
                                                {/* Approval Inbox — workflow approvers plus elevated admin oversight.
                                                    project_admin, project_member and org_admin remain blocked. */}
                                                <Route path="approvals/concerned-division" element={<RequireWorkflowRole role="division_approver" allowRoles={["super_admin", "admin"]}><ApprovalInboxConcernedDivision /></RequireWorkflowRole>} />
                                                <Route path="approvals/activity-owner" element={<RequireWorkflowRole role="division_approver" allowRoles={["super_admin", "admin"]}><ApprovalInboxActivityOwner /></RequireWorkflowRole>} />
