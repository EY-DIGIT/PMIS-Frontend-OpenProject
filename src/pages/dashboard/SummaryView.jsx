...
import {
  DonutChart, StackedBar, FunnelBars, GaugeRing, formatINR, DOMAIN,
} from "./charts";
...
export default function SummaryView() {
  ...
  useEffect(() => {
    ...
    fetchSummaryView({ delayMinDays: 1, topN: 5 })
      .then((payload) => {
        if (cancelled) return;
...
function SummaryContent({ data, onOpenProjectList, onOpenProjectItems, onOpenOrg, onOpenDivision, navigate }) {
  ...
  const counts = {
    total: status.total ?? 0, active: status.active ?? 0, completed: status.completed ?? 0,
    ontrack: status.ontrack ?? 0, delayed: status.delayed ?? 0,
  };
    ...
    <>
      ...
      {/* operations */}
        <Widget title="Tickets by Priority" sub={tickets.available ? `${tickets.total} tickets` : "support tickets"} onClick={() => navigate("/tickets")}>
          ...
          {tickets.available && (
            (tickets.byPriority || []).some((p) => p.total)
              ? <PriorityBars rows={(tickets.byPriority || []).map((p) => ({ label: p.label, total: p.total, open: p.open, color: p.color }))} />
              : <div className="dash-empty">No tickets found.</div>
          )}
        </Widget>
        {/* Meetings Overview — hidden from the dashboard per request.
...
function ProjectListView({ mode, searchText, onSearch, onBack, onOpenProject }) {
  ...
  useEffect(() => {
    let cancelled = false; setBusy(true); setErr(null);
...
function ProjectItemsInline({ project, status, onBack, navigate }) {
  ...
  useEffect(() => {
    ...
    fetchProjectItems(project.uuid, { bucket: status || undefined })
      .then((payload) => { if (cancelled) return; const rows = extractItemsPayload(payload); setItems(rows.map((r) => itemRowToTrackRow(r, project))); setBusy(false); })
      .catch(() => { if (cancelled) return; setErr("Failed to load project items. Please retry."); setItems([]); setBusy(false); });
    return () => { cancelled = true; };
  }, [project?.uuid, status]);
