...
    actualEnd: d(node.actualEnd) || (ae.length && ae.length === tasks.length ? new Date(Math.max(...ae)) : null),
  };
}
...
        wbs: t.displayCode || `T${mi + 1}.${ai + 1}.${ti + 1}`, name: t.name, context: parentName, node: t,
...
          wbs: s.displayCode || `ST${mi + 1}.${ai + 1}.${ti + 1}.${si + 1}`, name: s.name, context: t.name, node: s,
...
      wbs: m.displayCode || `M${mi + 1}`, name: m.name, context: project.name, node: m,
...
        wbs: a.displayCode || `A${mi + 1}.${ai + 1}`, name: a.name, context: m.name, node: a,
