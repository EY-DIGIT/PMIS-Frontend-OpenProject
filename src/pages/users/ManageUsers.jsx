    ...
    (async () => {
      try {
        ...
        /* Background fetch — org users for the Organization User section,
           one call per role from the authz assignable-users endpoint.
           Doesn't block render; each dropdown lights up as its list
           resolves.
             Project Admin → assignable-users/project_admin
             Project User  → assignable-users/project_member */
        ORG_USER_ROLES.forEach(({ roleLabel }) => {
          listAssignableUsers(projectId, roleLabel)
